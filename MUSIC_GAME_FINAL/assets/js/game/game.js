...
var PLAYER =
{
	...
	if(myparam == 0){
		...
		this.ms_Group.position.y = CONV.YTo3D( -0.6 );
		...
		if(myparam != 0)
		{
			DISPLAY.ms_Light.shadowDarkness = 0;
			DISPLAY.ms_CloseLight.shadowDarkness = 0;
		}
...
var GROUND =
{
	...
	{		
		...
		for( var i = 0; i <mystream.length; i += 1 )
		{
			...
			y = (mystream[i]+0.1)*2*(1-0.001*Math.random())-0.2;
			j+=0.33045/2;
			// var x1 = j, x2 = j +0.6;
			// y = mystream[i]*3*(1+0.1*Math.random());
			// j+=0.7609;
			// /alert(y);
...
var TREES =
{
	...
	GenerateTrees: function( inNbTrees )
	{
		...
		{			
			var x = ( 0.2 + Math.random() * 0.7 ) * GAME.ms_Parameters.widthSegments - GAME.ms_Parameters.widthSegments / 2;
			...
			//if( y > 15.0 )
			{
				...
				var aScale = Math.random() * 0.5 + 0.75;
...
var GAME =
{
	...
	// get the position of the object in B2D world
